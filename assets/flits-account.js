...
jQuery(function($){
  if( screen.width < 540 )
  {
    ...
    $('.flits-return-to-menu').on('click', function(){
      if( $(this).parent().find( '.flits-nav' ).length > 0 )
      {
        $('.flits-nav').toggle();
      }
      else
      {
      	$('.flits-nav').detach().prependTo( $(this).parent() ).show();
      }
    });

    //$('.flits-page-profile').removeClass('flits-hidden');
    //$('.flits-list-group a[data-href="#flits-page-profile"]').trigger('click');
  }
});