...
jQuery( function ($) {
	var Sections = {
		...
		_onSectionLoad: function( e ) {
			...
			function _loadCollectionTemplate( t ) {
				...
				$(window).on('resize', Reqs.debounce(Site.setBadgePosition, 250));
	...
	window.Site = {
		...
		header: function( enableHero ){
			var $header = $('.site-header'),
				scroll = 0,
				heroEnabled = enableHero != null ? enableHero : $('.bodyWrap').children().first().hasClass( 'header--full' ),
		...
		sliders: {
			...
			setSlidesHeight: function(slider){
				...
				slider.find( '.js-slide' ).each(function() {
					var $slide = $(this);
					var hasImage = $slide.find( 'img' ).length;
			...
			setCarouselState: function($container) {
				...
				if ( $carousel.length ) {
					...
					var windowWidth = window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth;
                  	...
                  	var slideCount = 4;
                  	var mobileSlideCount = 1; //2;
                  	if ( $container.hasClass( 'moreProducts' ) ) {
                    	slideCount = 4;
                    	mobileSlideCount = 2;
                    }

						if ( !isInitialized && !( $('body').hasClass( 'template-product' ) && windowWidth >= 768 ) ) {
							$carousel.slick({
                      ...
                      if ($("body").hasClass("template-index") || $("body#about").length > 0 ) {
                        var $container 	  = $('.collection-slider__wrapper');
                        var $headingpos = $(".collectionGrid h3").offset();
                        $container.css( { marginLeft : $headingpos.left + "px" } );
                      }
                    });
				...
				$(document).ready(function() {
					// MOVE CUP SIZE GUIDE INSIDE SIZE LABEL
					if ( $('#cupsizebutton').length ) {
						var sizebutton = $('#cupsizebutton');
                      
                      	
						if ( $('label[for="product-select-option-0"]').length ) {
							$('label[for="product-select-option-0"]').append(sizebutton);
						}
					}
                  
                  	
                  	if($(".everyday__template").length > 0){
                      
                      	var size_chart = $("#size__chart-container").html();
                                    	
                        $(".everyday__template").find(".selector-wrapper label").each(function(){
                            var label_val = $(this).text();

                            if(label_val == "Size" || label_val == "size"){
                                $(this).append(size_chart)
                            }
                        });
                    }
			...
			logosList: function(){
          	...
          	/* Product Slider - Extra Images next to tabs */
			tabsList: function(){
				var self = this;
				var slider = $('.js-tabs-slider .images');
				var options = {
					'infinite': true,
					'autoplay': false,
					'speed': 300,
				...
				slider.each(function() {
              	...
              	$('.js-tabs-slider span.prev').on('click', function () {
                	$('.js-tabs-slider .images').slick('slickPrev');
                });
              	$('.js-tabs-slider span.next').on('click', function () {
                	$('.js-tabs-slider .images').slick('slickNext');
                });
              
              	var heightofslider = $('.template-product .product-info-extra .js-tabs-slider .images').outerHeight();
              	var heightoftabs = $('.template-product .product-info-extra .tab-holder').outerHeight();
                if ( heightofslider < heightoftabs ) {
                  $('.template-product .product-info-extra').css('min-height', heightoftabs);
                } else {
                	$('.template-product .product-info-extra').css('min-height', heightofslider);
                }
			},
          
          	/* Product Slider - How-to blocks */
			howToSlides: function(){
              
              if ( $( ".js-how-to-slider" ).length ) {
                
				var self = this;
				var slider = $('.js-how-to-slider');
				var options = {
					'infinite': false,
					'autoplay': false,
					'speed': 300,
                  ...
                  var thirdSlide = $('.block-holder .slick-slide[data-slick-index="2"]');
                  var thirdSlideRight = ($(window).width() - (thirdSlide.offset().left + thirdSlide.outerWidth()));
                  console.log('Third slight is at position ' + thirdSlideRight);
                  $('#how-to-slider-next').css('right', thirdSlideRight);
                });
              	$(window).resize(function() {
                  var thirdSlide = $('.block-holder .slick-slide[data-slick-index="2"]');
                  var thirdSlideRight = ($(window).width() - (thirdSlide.offset().left + thirdSlide.outerWidth()));
              	...
              	$('#how-to-slider-next').on('click', function (e) {
                  	e.preventDefault();
                	$('.js-how-to-slider').slick('slickNext');
                });
				slider.each(function() {
                  	$( this ).slick( options );
				});
			...
			product: function(){
				...
				var sliderNavArrows = $productImgSliderNav.find('.js-slide').length > 1;
				...
				if ( $productImgSlider.find('.js-slide').length > 1 ) {
					...
					$productImgSliderNav.flickity({
                      ...
                      $productImgSlider.flickity('previous');
                    });

                    $('.flickity-button.next').on( 'click', function() {
                      $productImgSlider.flickity('next');
                    });
				}
			...
			productTabsProducts: function(){
				var $productTab = $('.product-info-extra .tab h4');
              	$productTab.click(function (index, value) {
                	$(this).parent('.tab').toggleClass('toggled');
                });
		...
		nav: {
			...
			show: function(){
				var navMenuWidth = $('.nav-inner').width() + 'px';
		...
		images: {
			loadBackgrounds: function() {
				...
				$elementsToLoad.each(function() {
					...
					if(src == '') {
						src = '//scarletperiod.com/cdn/shop/t/56/assets/placeholder-pattern.png?v=183063600167011207251698294818';
						placeholder = true;
					}
					...
					$(im).on('error', function() {
						$el.css('background-image', 'url(//scarletperiod.com/cdn/shop/t/56/assets/placeholder-pattern.png?v=183063600167011207251698294818)')
	...
	Cart = {
		init: function(){
			...
			$('.js-continueShopping').on('click', function(e){   
          	...
          	/*$('#Cart').on('click', '.js-continueShopping', function(e){ <!-- ABC */
				e.preventDefault();
				...
				if ( ($(window).width() > 768) && !(window.location.href.indexOf('/cart') > -1) ){                  
					/*-- ABC --*/
                  	...
                  	$('.cartToggle-items').trigger('click');
                  	/*-- /ABC --*/
				} else {                  
					if ( referrer === origin ) {                      
		...
		buildCart: function (cart) {
			...
			if (cart.item_count === 0) {
				$cart.append('<div class="cart-title block s1">\
								...
								<a href="/collections/shop-all" tabindex="-1" class="js-continueShopping js-continueShopping--empty js-go-back button">Shop All Products</a>\
							</div>');
			...
			$.each(cart.items, function(index, cartItem) {
		        ...
		        if ( discountsArray.length ) {
		            ...
		            for ( var j = 0; j < discountsArray.length; j++ ) {
                      	...
                      	discounts += '<p>' + discountsArray[j].title + '</p>';
                      	/*---- /ABC ----*/
		            }
		            discounts += '</div>';
		        }
			...
			Cart.updateCartTotal();
          	...
          	if(cart.total_price < country_price){
              if($(".static__bar-annunce .alert-title.less-price").length > 0 &&
                 $(".static__bar-annunce .alert-title.grater-pirce").length > 0
                ){
                	var remainingAmount = country_price - cart.total_price;
                	$(".ship__remaining").html(Shopify.formatMoney(remainingAmount));
                	$(".static__bar-annunce .alert-title.less-price").css("display", "block");
                	$(".static__bar-annunce .alert-title.grater-pirce").css("display", "none");
              }
            }else{
              if($(".static__bar-annunce .alert-title.less-price").length > 0 &&
                 $(".static__bar-annunce .alert-title.grater-pirce").length > 0
                ){
                	$(".static__bar-annunce .alert-title.less-price").css("display", "none");
                	$(".static__bar-annunce .alert-title.grater-pirce").css("display", "block");
              }
            }
            ...
            /*--------- ABC -----------*/            
            if( typeof abc_cart_freebie === 'function' )
            {              
              abc_cart_freebie( cart, Cart );                      	
            }
            /*--------- /ABC -----------*/
		...
		inputCounter: function( el, value ) {
			...
			$.ajax( {
				...
				success: function( cart ) {
					Cart.buildCart( cart );
                  	...
                  	if($(".ajax-load-price").length > 0){
                    	$(".ajax-load-price").find(".cart-total-price span").html(Shopify.formatMoney(cart.total_price));
                  	}
				}
			});
			...
			if ( qty == 0 ) {
          	...
          	/*--------- ABC -----------*/            
            if( typeof abc_event_item_removed === 'function' )
            {              
              abc_event_item_removed( id );                      	
            }
            /*--------- /ABC -----------*/
		},
									...
									'<div class="row inline">' +
										...
										'<div class="quickView-info block s12 xl_s12">' +
											...
											'<form class="productForm" action="/cart/add" method="post" data-color_swatches="' + enableColorSwatches + '" data-product_id="' + id + '">' +
												'<h2><a class="js-productUrl" href="' + url + '">' + title + '</a></h2>' +
												...
												'<div class="productForm-block">' +
													quantitySelect +
													'<div><button class="js-productForm-submit productForm-submit" type="submit" data-handle="' + product_handle + '" data-ajax="true">' + "Add To Cart" + '</button></div>' +
												'</div>' +
											'</form>' +
										'</div>' +
									'</div>' +
...
var Product = {
	init: function( update ){
		...
		if ( showShopBar ) {
      	...
      	if ( $('.product-how-to').length ) {
          	var headingpos = $('img.logo').offset();
          		headingpos = headingpos.left - 20;
            $('section.product-how-to .slider_holder').css({paddingLeft:headingpos+"px"});
        }
      
      	if ( $( "#detail-tabs" ).length ) {
          console.log('detailTabs exists');
          $( "#detail-tabs" ).tabs();
        }
	...
	callback: function(options){
		...
		if (variant) {
			if (variant.available) {
              	...
              	if($submit.hasClass("js-price-inherit")){
                	$submit.removeClass( 'is-disabled' ).prop( 'disabled', false ).html("Add To Cart" + ' - ' + Shopify.formatMoney(variant.price, moneyFormat) );
              	...
              	$(".stock__notification").css({'display': 'none'});
			...
			// this variant sold out
				...
				$price.css({'opacity': 0.3});
              	...
              	$(".stock__notification").css({'display': 'inline-block'});
...
var RelatedProducts = {
    init: function() {
    	var $container = $( '[data-section-type="related-products"]' );
      	if($('[data-section-type="related-products"]').length){
...
var Blog = {
	init: function() {
		...
		if (showFeatured) {
      	...
      	// MIKE BLOG CAROUSEL HERE
      	var $container 	  = $('.blogModule-posts__wrapper');
      	var $blogcarousel = $container.find( '.js-collection-slider' );
      	...
      	$('#journal-slider-next').on('click', function (e) {
        	e.preventDefault();
            $('.blogModule-posts__wrapper .js-collection-slider').slick('slickNext');
        });
      	$(document).ready(function() {
          if ( $( "#journal-heading" ).length ) {
      		var $headingpos = $("#journal-heading").offset();
            $container.css( { marginLeft : $headingpos.left + "px" } );
          }
        });
...
var Press = {
	init: function() {
		var $presscontainer = $('.scarlet-press-section');
      	var $presscarousel  = $presscontainer.find( '.js-collection-slider' );
	  	$presscarousel.slick({
            slidesToShow: 1,
            slidesToScroll: 1,
			infinite: true,
...
var FAQscroll = {
  init: function() {
  		...
  		$( ".scroll-buttons a.button" ).click(function(e) {
          e.preventDefault();
...
var qtyform = {
	init: function() {
      $('.quantity-holder #increase').on('click', function(e) {
      	var existingqty = $('.quantity-holder #quantity').val();
        	existingqty = parseInt(existingqty);
        var newqty = parseInt(existingqty + 1);
        if (newqty < 10) {
          $('.quantity-holder #quantity').val(newqty);
        }
      });
      $('.quantity-holder #decrease').on('click', function(e) {
      	var existingqty = $('.quantity-holder #quantity').val();
        	existingqty = parseInt(existingqty);
        var newqty = parseInt(existingqty - 1);
        if (newqty > 0) {
...
//--- ABC
          jQuery(function($){
            //-- if Checkout page, remove all click event listeners
            if( $('#your-shopping-cart').length )
            {
              $('.js-cartToggle').off('click')/*.parent().css('display', 'inline-block')*/;
              /*
              $('#Cart').on('click', '.js-cartToggle', function(){
              	var referrer = document.referrer;
				var origin = window.location.origin + '/';
            ...
            $('#Cart').on('click', '.js-continueShopping--empty', function(){
              window.location.href="/collections/shop-all";
            });
            
            abc_show_hide_boxes_cart_page();
            
            //-- change button text in Tampons page to "Add To Cart"
            $('#100-organic-cotton-tampons-subscription-scarlet .productForm-submit').text('Add To Cart');
          });
          
          
          //-- show or hide cart page's boxes
          function abc_show_hide_boxes_cart_page(){
            if( $('#your-shopping-cart #CartItems .line-item').length <= 0 )
            {
              $('#your-shopping-cart .cart-details').hide();
              $('#your-shopping-cart .cbb-also-bought-container').hide();

              $('#your-shopping-cart .cart-home').addClass('abc_empty_cart_page');
            }
            else{
              $('#your-shopping-cart .cart-details').show();
              $('#your-shopping-cart .cbb-also-bought-container').show();

              $('#your-shopping-cart .cart-home').removeClass('abc_empty_cart_page');
            }
            
            //-- display the Close button, if not present
            if( ! $('.js-cartToggle.js-cartToggle-close').length )
            {
              $('#Cart').prepend('<a style="width: 30px;height: 30px;margin-bottom: 20px;"href="/"><div class="js-cartToggle js-cartToggle-close cart-close icon-close diamond-outline"><div class="diamond"></div></div></a>');
            }
          }
          
          /*
          var ABC_user_removed_freebie = false,
              freebie_trigger_amount = 6000, 			//-- 60$ 
              freebie_variant_id = 31648099401763;	//-- freebie item's variant id
          
          //-- when an item is remove
          function abc_event_item_removed(id)
          {
            var variant_id = id.split('_')[1];
            if( variant_id == freebie_variant_id )
            {
              ABC_user_removed_freebie = true;
              console.log('user remove freebie');
            }
            console.log('item remove = ' + variant_id);
          }
          
          
          //-- to add freebie, based on cart total
          function abc_cart_freebie( cart, Cart )
          {
            
            //abc_move_freebie_to_bottom();
            
            //-- if user removed the freebie, then no need to add it again
            if( ABC_user_removed_freebie ) return;
            
            //-- check already added or not
            var has_freebie_added = false;              
            if( cart.items.length > 0 )
            {
              for( var i = 0; i < cart.items.length; i++ )
              {
                if( cart.items[i].id == freebie_variant_id )
                {
                  has_freebie_added = true;
                  break;
                }
              }
            }
            ...
            //-- check whether total amount is eligible for freebie
          	if( cart.total_price >= freebie_trigger_amount )
            {
              console.log('>= 60$');
              console.log(cart);
                            
              
              if( has_freebie_added )
              {
                //-- freebie alreasdy exists, do nothing
            ...
            {                            
              if( has_freebie_added )
              {
                ...
                jQuery.post('/cart/update.js', {updates: {freebie_variant_id: 0}});
              }
              else
              {  
                console.log('no_need_to_remove');
              }
            } 
          }
          ...
          //-- move freebie to the bottom
          function abc_move_freebie_to_bottom()
          {            
            if( $('#CartItems .line-item[data-variant-id="'+freebie_variant_id+'"]').length )
            {
              //$('#CartItems .line-item[data-variant-id="'+freebie_variant_id+'"]').appendTo('#CartItems');
              
              $('#CartItems .line-item[data-variant-id="'+freebie_variant_id+'"] .cart-product-discounts p').html('Free Gift! ❤️');
            }
          }
...
$(document).ready(function(){
  	...
  	$(".trigger__btn-container.size__toggle a").on("click", function() {
	
		$(this).css("display", "none");
      	$(this).closest(".product__variant-toggle").find(".slide__size").css("display", "inline-block");
        //$(this).closest(".product__variant-toggle").find("form .js-productForm-submit").trigger("submit");                                         
    })
    
    
    
    /*$(".product__variant-toggle").on("mouseout", function(){
    	$(this).find(".trigger__btn-container.size__toggle a").css("display", "block");
      	$(this).find(".slide__size").css("display", "none");
    })*/
    
    $(".product__variant-toggle").hover( function() {
  	...
  	/* Altranetive varinat selection section */      
	$(".product__variant-lists .slide__color .swatch input").on("click", function(){
      	      
  		var selected_color = $(this).val();
      	var size_val = "";
      	
      	$(this).closest(".product__variant-lists").find(".slide__size input").each(function(){
        ...
        if(size_val != ""){
      		var variant_name = size_val+" / "+selected_color;
        }else{
          	var variant_name = selected_color ;   
		}
      
      	$(".product__variant-lists form #product-select > option").each(function(){
      		var variants = $.trim($(this).text());
          	var current_variant = $(this).val();
          	if(variants.indexOf(variant_name) != -1){
           		$(this).parent("#product-select").val(current_variant);
            }
      	})
    })
    ...
    function ContainsExactWord(sentence, compare) {
      ...
      console.log("Could not find the word "+sentence);
      return false;
    }
    
    $(".product__variant-lists .slide__size .swatch input").on("click", function() {
		
      	var selected_color = "";
      	var size_val = $(this).val();
      
      	$(this).closest(".product__variant-lists").find(".slide__color input").each(function(){
        ...
        if(selected_color != ""){
      	...
      	/*$(".product__variant-lists form #product-select > option").each(function(){
          	...
          	console.log(variants+" - "+variant_name);
          	if(variants.indexOf(variant_name) != -1){
           		$(this).parent("#product-select").val(current_variant);
            }
      	})*/
        
        $(this).closest(".product__variant-lists").find("form #product-select > option").each(function(){
      		var variants = $.trim($(this).text());
          	var current_variant = $(this).val();
          
          
          	if(ContainsExactWord(variants, variant_name)){
              	console.log(variants+" - "+variant_name);
           		$(this).parent("#product-select").val(current_variant);
            }
      	...
      	$(this).closest(".product__variant-toggle").find("form .js-productForm-submit").trigger("click");
  	...
  	$(".nav-standard .menu").find("li a").each(function(){
      
      	var current_nav = $(this);
  		var data_rel = current_nav.attr("data-dropdown-rel");
      
      	if(data_rel != ""){
          	$(".mega-menu-container").find(".mega-menu-section").each(function(){
          		var active_dropdown = $(this);
              
              	$(active_dropdown).find(".mega-menu").each(function(){
              		var mega_menu = $(this);
                  
                    active_attr = mega_menu.attr("data-dropdown");
                  
                  	if(active_attr == data_rel){
                  	  	mega_menu.clone().insertAfter(current_nav);
                  	}
              	})
          	})
      	}
    ...
    $('#tabs-nav li').click(function(){
  	...
  	$(window).scroll(function (event) {
      	...
      	if(scroll > 80){
        	$(".site-header-right .nav-standard.nav-main").removeClass("active-nav");
        }else{
          	$(".site-header-right .nav-standard.nav-main").addClass("active-nav");
        }
	...
	$.fn.isInViewport = function() {
      ...
      console.log(elementBottom+" - "+elementTop+" - "+viewportTop+" - "+viewportBottom);    
    ...
    $(".stock__notification").on("click", function(){
     	$(document).find(".BIS_trigger").trigger("click");
        return false;
	})      
})  
...
function checkCookie() {
  let user = getCookie("newsletter-popup");
  ...
  if (user != "") {
   ...
   // alert("Welcome again " + user);
    ...
    console.log('entr');
     ...
     setCookie('newsletter-popup', 'popped', 7);
  }
}
      jQuery(document).ready(function(){
      //checkCookie();
         setTimeout(function(){ checkCookie(); }, delay);
...
$(document).ready(function(){
  function popupStateCheck(){
    console.log('popupStateCheck')
    var currentDate = $('.getDateFromDB').html();
    var timeDiff = 0; 
    var diffDays = 0;
    var getCookieValue = localStorage.getItem("popupShow");
    console.log(getCookieValue);
    if(getCookieValue){
      timeDiff = currentDate - getCookieValue;
      diffDays = ((timeDiff/3600)/24); 
      if(diffDays < 7){
        $('#newsletter-overlay').hide();
      }
    }else{
      $('#newsletter-overlay').show();
      localStorage.setItem('popupShow', "");
    }
  ...
  function popupCloseBtnClick(){
    ...
    var currentDate = $('.getDateFromDB').html();
    localStorage.setItem('popupShow', currentDate);
    $('#newsletter-overlay').hide();
  }
  
  $('.popup-close').off('click').on('click',popupCloseBtnClick);
  var popupTimer = setTimeout(function(){
      $('#newsletter-overlay').removeClass('dispnone')
  	  popupStateCheck();
